...
const FeaturedCollections = ({ products = [] }) => {
    ...
    return (
        <section className="py-16 bg-white w-full justify-center scrollbar-hide">
            <div className="max-w-7xl px-4 mx-auto sm:px-6 lg:px-8 overflow-y-hidden overflow-x-auto">
                {/* Section Header */}
                <div className="text-center mb-12">
                    ...
                    <p className="text-gray-600 text-lg">
                        Discover our handpicked essentials
                    </p>
                </div>
                ...
                <div className="hidden lg:grid lg:grid-cols-3 xl:grid-cols-6 gap-76">
                    {featuredProducts.map((product) => (
                        <Link key={product.id} to={`/product/${product._id || product.id}`}>
            ...
            >
                <Heart
                    className={`w-4 h-4 transition-all duration-200 ${
                        isInWishlist(product.id || product._id) ? 'text-red-500 fill-red-500' : 'text-gray-600'
                    }`}
                />
            </button>
